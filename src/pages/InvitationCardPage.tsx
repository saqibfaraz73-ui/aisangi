...
import { Download, Heart, Palette, Plus, Type, Trash2, Upload, Image as ImageIcon } from "lucide-react";
import { toast } from "sonner";

const INVITATION_TEMPLATES = POSTER_TEMPLATES.filter((t) => t.category === "Invitation Card");
...
export default function InvitationCardPage() {
  ...
  const [elements, setElements] = useState<TemplateElement[]>(INVITATION_TEMPLATES[0].elements);
  const [selectedSize, setSelectedSize] = useState<PosterSize>(POSTER_SIZES[0]);
  const [selectedElement, setSelectedElement] = useState<string | null>(null);
  const [uploadedPhotos, setUploadedPhotos] = useState<Record<string, string>>({});
  const [bgColor, setBgColor] = useState(INVITATION_TEMPLATES[0].bgColor);
  ...
  const selectTemplate = (t: typeof INVITATION_TEMPLATES[0]) => {
  ...
  const addTextElement = () => {
    ...
    setElements((prev) => [...prev, {
      id, type: "text", x: 10, y: 50, width: 80, height: 15,
      text: "Your Text Here", fontFamily: "Inter", fontSize: 3, fontWeight: "normal",
      color: "#FFFFFF", textAlign: "center", direction: "ltr", editable: true, lineHeight: 1.4,
    }]);
  ...
  const addImageElement = () => {
    ...
    setElements((prev) => [...prev, {
      id, type: "image", x: 20, y: 20, width: 30, height: 30,
      editable: true, isPhoto: true, borderRadius: 0, bgColor: "rgba(255,255,255,0.1)",
    }]);
    setSelectedElement(id);
  ...
  const deleteElement = (id: string) => {
    ...
    setUploadedPhotos((prev) => { const next = { ...prev }; delete next[id]; return next; });
  ...
  const exportPoster = async (format: "png" | "jpg") => {
    ...
    if (bgImage) {
      const bgImg = await new Promise<HTMLImageElement>((resolve) => {
        ...
        img.onload = () => resolve(img); img.onerror = () => resolve(img); img.src = bgImage;
      });
      if (bgImg.complete && bgImg.naturalWidth > 0) {
        const imgRatio = bgImg.width / bgImg.height; const boxRatio = pw / ph;
    ...
    for (const el of elements) {
      const x = (el.x / 100) * pw, y = (el.y / 100) * ph;
      const w = (el.width / 100) * pw, h = (el.height / 100) * ph;
      if (el.type === "rect") {
        ctx.fillStyle = el.bgColor || "#333"; ctx.globalAlpha = el.opacity ?? 1;
        ...
        ctx.fillStyle = el.color || "#FFFFFF"; ctx.globalAlpha = el.opacity ?? 1;
        ...
        ctx.direction = "ltr"; ctx.globalAlpha = 1;
      }
    }
    ...
    canvas.toBlob((blob) => {
      ...
      a.download = `invitation-card-${selectedSize.label.replace(/\s/g, "-")}.${format}`;
  ...
  const filteredSizes = sizeCategory === "Custom" ? [] : POSTER_SIZES.filter((s) => s.category === sizeCategory);
      ...
      <main className="max-w-7xl mx-auto p-3 sm:p-4 overflow-x-hidden">
        <div className="text-center mb-4 sm:mb-6">
          <h2 className="text-lg sm:text-2xl font-display font-bold text-foreground flex items-center justify-center gap-2">
            ...
            Invitation Card Maker / دعوت نامہ
          </h2>
          <p className="text-xs sm:text-sm text-muted-foreground mt-1">
            Wedding, Nikah, Walima, Mehndi, Baraat, Party & Business invitation cards
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
          ...
          <div className="space-y-3 min-w-0">
            ...
            <p className="text-[10px] sm:text-xs text-muted-foreground">
              Click on card or select below to edit. Use sliders to position.
            </p>
            {currentElement ? (
            ...
            <div className="space-y-1">
              ...
              {elements.filter((e) => e.editable).map((el) => (
                <div key={el.id} className="flex items-center gap-1">
                  <button onClick={() => setSelectedElement(el.id)}
                  ...
                  {el.id.startsWith("custom-") && (
                    <button onClick={() => deleteElement(el.id)} className="p-1 text-muted-foreground hover:text-destructive transition-colors">
